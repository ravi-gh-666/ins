...
function renderScorecard(data) {
    ...
    <div class='card bg-white rounded-lg shadow p-6 mt-6'>
        ...
        ${(d.mandatory_features||[]).map(f=>`<div class='card bg-gray-50 rounded-lg shadow-sm p-4 mb-4 flex flex-col md:flex-row gap-6 items-stretch'><div class='flex-1 flex flex-col justify-center'><div class='feature-title flex items-center text-base font-semibold mb-2'><span class='material-icons feature-icon text-blue-700 mr-2'>verified_user</span>${f.FeatureName}</div><p class='mb-1'><strong>${tr("Offered:")}</strong> <span class='tag ${f.FeatureOffered=="Yes"?"green":(f.FeatureOffered=="No"?"red":"orange")}' style='margin-left:8px;'>${f.FeatureOffered}</span></p>${f.Explanation?`<div class='explanation text-gray-700 mb-1'>${f.Explanation}</div>`:''}${f.Details?`<div class='details text-gray-600 mb-1'><strong>${tr("Details:")}</strong> ${f.Details}</div>`:''}${f.why_matters?`<div class='why-matters text-blue-700 italic mb-1'><strong>${tr("Why this matters:")}</strong> ${f.why_matters}</div>`:''}${f.Caveats?`<div class='caveats text-red-600 mb-1'><strong>${tr("Caveats:")}</strong> ${f.Caveats}</div>`:''}</div>${featureYoutubeEmbed(f)}</div>`).join('')}
        ...
        <h3 style='font-size:2rem;font-weight:400;color:#202124;margin-bottom:1.2rem;'>${tr("Additional Features")}</h3>
        ${(d.policy_data.Addons||[]).map(a=>`<div class='card bg-yellow-50 rounded-lg shadow-sm p-4 mb-4 flex flex-col md:flex-row gap-6 items-stretch'><div class='flex-1 flex flex-col justify-center'><div class='feature-title flex items-center text-base font-semibold mb-2'><span class='material-icons feature-icon text-orange-600 mr-2'>add_circle</span>${a.name}</div><div class='explanation text-gray-700 mb-1'>${a.description||''}</div>${a.details?`<div class='details text-gray-600 mb-1'><strong>${tr("Details:")}</strong> ${a.details}</div>`:''}</div></div>`).join('')}
